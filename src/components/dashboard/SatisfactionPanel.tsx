          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
              ...
              <h3 className="font-display font-semibold mb-4 flex items-center gap-2">
                <Building2 className="h-4 w-4 text-primary" /> Nota por Escola
              </h3>
              <ResponsiveContainer width="100%" height={Math.max(200, schoolRatings.length * 40)}>
                <BarChart data={schoolRatings.slice(0, 10)} layout="vertical">
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                  <XAxis type="number" domain={[0, 5]} tick={{ fontSize: 12 }} />
                  <YAxis type="category" dataKey="escola" tick={{ fontSize: 10 }} width={180} />
                  <Tooltip formatter={(v: number) => [`${v}/5`, "Nota Média"]} />
                  <Bar dataKey="avg" radius={[0, 8, 8, 0]}>
                    {schoolRatings.slice(0, 10).map((s, i) => (
                      <rect key={i} fill={s.avg >= 4 ? "hsl(142, 73%, 28%)" : s.avg >= 3 ? "hsl(38, 92%, 50%)" : "hsl(0, 84%, 60%)"} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
